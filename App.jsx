...
const BACKLOG_SCHED_START = "2026-08-20";  // day backlog clearing begins
...
function buildBacklogSchedule() {
  ...
  let usedToday = 0;
  let quotaToday = isSunday(cursorDate) ? 3 : 2;
  ...
  SUBJECT_ORDER.forEach(subj => {
    bySubj[subj].forEach(task => {
      if (usedToday >= quotaToday) {
        ...
        usedToday = 0;
        quotaToday = isSunday(cursorDate) ? 3 : 2;
      }
...
const BACKLOG_TASKS = buildBacklogSchedule();          // permanent, deterministic
...
const ALL_BACKLOG_END_DATE = BACKLOG_TASKS.length ? BACKLOG_TASKS[BACKLOG_TASKS.length - 1].scheduledDate : BACKLOG_SCHED_START;
...
/* ============================================================
   SETTINGS
   ============================================================ */
function SettingsPage({ themeMode, onChangeTheme, dailyTargetHours, onChangeTarget, examDate, onChangeExamDate, onExportData }) {
  return (
    <div>
      ...
      <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text-muted)", marginBottom: 8 }}>DATA</div>
      <div style={{ background: NAVY_CARD, borderRadius: 14, padding: 14, marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 10.5, color: "var(--text-muted)", marginTop: 8 }}>Downloads every stored record — progress, hours, history, mistakes, tests — as one file you can keep safe.</div>
      </div>

      <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text-muted)", marginBottom: 8 }}>ABOUT</div>
      <div style={{ background: NAVY_CARD, borderRadius: 14, padding: 14 }}>
        <div style={{ fontSize: 12, color: "var(--text-dim)" }}>NEET 2027 Command Center</div>
        <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4 }}>{PLANNER.length} planner lectures loaded · data stored on your Claude account, private to you.</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ ...rootVars, minHeight: "100vh", background: "var(--bg)", fontFamily: "Inter, system-ui, sans-serif", paddingBottom: 76 }}>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "12px 16px" }}>
        ...
        {tab === "more" && moreTab === "settings" && (<><SubPageHeader title="Settings" onBack={() => setMoreTab(null)} /><SettingsPage themeMode={themeMode} onChangeTheme={onChangeTheme} dailyTargetHours={dailyTargetHours} onChangeTarget={onChangeTarget} examDate={examDate} onChangeExamDate={onChangeExamDate} onExportData={onExportData} /></>)}
